rootApp.controller('topBarCtrl', ["$scope","$rootScope", "$state", "$http", "$stateParams","locals",function ($scope,$rootScope, $state, $http, $stateParams,locals) {
    ...
    //头部我的讲课我的听课切换
    if(location.href.indexOf('/app/ktgc/main/1')>1){
        $(".topnav li").eq(0).addClass("active").siblings().removeClass("active");
    }else if(location.href.indexOf('/app/ktgc/main/2')>1){
        $(".topnav li").eq(1).addClass("active").siblings().removeClass("active");
    }
    $scope.topChangeTab = function (index) {
        $(".topnav li").eq(index - 1).addClass("active").siblings().removeClass("active");
        $state.go("app.ktgc.main1",{tkjk:index})
    }
